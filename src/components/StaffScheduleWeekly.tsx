...
// Helper functions for weekly view logic
...
export function StaffScheduleWeekly({ onBack }: StaffScheduleWeeklyProps) {
  ...
  const [schedules, setSchedules] = useState<Record<string, StaffSchedule>>(() => {
    ...
    // Initialize current month with varied data
    STAFF_DATA.forEach((staff, staffIdx) => {
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Vary the schedules for demo purposes
        const isPresent = Math.random() > 0.2; // 80% present
        const shiftCount = isPresent ? Math.floor(Math.random() * 3) + 1 : 0; // 1-3 shifts
  ...
  const weeklyStats = useMemo(() => {
    ...
    if (stats.totalWorkingDays > 0) {
      ...
      stats.avgAbsent = Math.round(stats.avgAbsent / stats.totalWorkingDays);
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#FFE5E8] to-[#FFF5F6] pb-6">
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <p className="text-base font-semibold text-[#1A1A1A]">{formatMonthYear(currentDate)}</p>
            <p className="text-xs text-[#6B7280]">Week {currentWeekIndex + 1} of {totalWeeks}</p>
          </div>
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-4 py-3">
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            <p className="text-xs text-[#6B7280] mb-1">Avg/Day</p>
            <p className="text-lg font-semibold text-[#1A1A1A]">{weeklyStats.avgPresent}</p>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="px-2 py-0.5 rounded text-xs font-medium bg-[#D1ECF1] text-[#0C5460]">
                        {attendanceRate}%
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-4 mb-3">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-[#6B7280] mb-1">Shift Types</p>
                    <p className="text-sm font-semibold text-[#1A1A1A]">
                      {weekData.uniqueShifts.length}
                    </p>
                  </div>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {isPresent && schedule.shifts.length > 0 && (
                            <span className="text-xs font-semibold text-[#28A745] mt-0.5">
